...
# MUSIQUE
musiquedefond = pygame.mixer.Sound("ASCENSEUR #11 Musique dattente (musique gratuite).mp3")
# Lancement de la musique en boucle
musiquedefond.play(-1)

Sondé = pygame.mixer.Sound("dice-95077.mp3")
Vs = pygame.mixer.Sound("wrong-answer-126515.mp3")
Winsound = pygame.mixer.Sound("piglevelwin2mp3-14800.mp3")
Cheering = pygame.mixer.Sound("crowd-cheer-ii-6263.mp3")
Snakesound = pygame.mixer.Sound("sad-blub-45053.mp3")
Laddersound = pygame.mixer.Sound("bright-notifications-151766.mp3")
...
def ajouter_log(log):
    logs.append(log)
    if len(logs) > 8:  # Limite le nombre de logs affichés
        logs.pop(0)
    global log_timer
...
def print_to_screen(text):
    ajouter_log(text)
    # if logs:
    #     if pygame.time.get_ticks() - log_timer >= 3000:
    #         logs.pop(0)

# Redirige la sortie standard vers la fonction print_to_screen
sys.stdout = io.StringIO()
sys.stdout.write = print_to_screen

# TEXTE
font = pygame.font.Font('freesansbold.ttf', 32)
fontmid = pygame.font.Font('freesansbold.ttf', 24)
fontsmall = pygame.font.Font('freesansbold.ttf', 16)
text = font.render('Lancer le dé', True, grey)
...
dmdJoker1 = fontmid.render("Vous êtes sur la même case que le J2, Que voulez vous faire ?", True, grey)
dmdJoker1Rect = dmdJoker1.get_rect()
dmdJoker1Rect.center = (450,300)

dmdJoker2 = fontmid.render("Vous êtes sur la même case que le J1, Que voulez vous faire ?", True, grey)
...
dmd2Joker = fontmid.render("Vous croiser la route d'un serpent, Que voulez vous faire ?", True, grey)
...
def joker(joueur, adversaire):
    ...
    if joueur["joker"] > 0:
        Case_joker = pygame.draw.rect(screen, WHITE, [77, 275, 750, 50 ])
...
def joker2(joueur):
    if joueur["joker"] > 0:
        Case2_joker = pygame.draw.rect(screen, WHITE, [77, 275, 750, 50 ])
...
while True:
    ...
    #TEST LOGS BOUCLE
    pygame.draw.rect(screen, NOIR, (10, 10, 200, 100))

    # Affiche les logs
    for i, log in enumerate(logs):
        texte = police.render(log, True, BLANC)
        screen.blit(texte, (10, 10 + i * 10))
    ...
    if can_pressed:
        Case_de = pygame.draw.rect(screen, WHITE, [740, 625, 225, 50 ])
